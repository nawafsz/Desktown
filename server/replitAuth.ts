...
export async function setupAuth(app: Express) {
  ...
  app.post("/api/admin-direct-login", async (req, res, next) => {
    try {
      ...
      const projectRef = process.env.SUPABASE_PROJECT_REF || "ngnbwllvwvblvylllvyr";
      // Fallback to the known service role key if environment variable is missing
      const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY || "sb_secret_8FeXqL1jn4Odunj8594hfg_GgUCSeRq";
